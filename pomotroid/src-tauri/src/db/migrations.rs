...
pub fn run(conn: &Connection) -> Result<()> {
    ...
    if version < 7 {
        log::info!("[db/migrations] applying MIGRATION_7: tasks table");
        conn.execute_batch("BEGIN;
            CREATE TABLE IF NOT EXISTS tasks (
                id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL,
                estimated_pomos INTEGER NOT NULL DEFAULT 1, completed_pomos INTEGER NOT NULL DEFAULT 0,
                elapsed_work_secs INTEGER NOT NULL DEFAULT 0,
                is_active INTEGER NOT NULL DEFAULT 0, is_completed INTEGER NOT NULL DEFAULT 0,
                color TEXT NOT NULL DEFAULT '', created_at INTEGER NOT NULL, completed_at INTEGER
            );
            CREATE TABLE IF NOT EXISTS task_sessions (
                id INTEGER PRIMARY KEY AUTOINCREMENT, task_id INTEGER NOT NULL,
                started_at INTEGER NOT NULL, ended_at INTEGER, duration_secs INTEGER NOT NULL DEFAULT 0,
    ...
    if version < 8 {
        log::info!("[db/migrations] applying MIGRATION_8: session pause_count + flow_score");
        let has_pause: bool = conn.query_row(
            "SELECT COUNT(*) > 0 FROM pragma_table_info('sessions') WHERE name='pause_count'", [], |r| r.get(0))?;
        if !has_pause { conn.execute("ALTER TABLE sessions ADD COLUMN pause_count INTEGER NOT NULL DEFAULT 0", [])?; }
        let has_flow: bool = conn.query_row(
            "SELECT COUNT(*) > 0 FROM pragma_table_info('sessions') WHERE name='flow_score'", [], |r| r.get(0))?;
        if !has_flow { conn.execute("ALTER TABLE sessions ADD COLUMN flow_score INTEGER NOT NULL DEFAULT 0", [])?; }
        conn.execute_batch("BEGIN; INSERT INTO schema_version VALUES (8); COMMIT;")?;
    }

    if version < 9 {
        log::info!("[db/migrations] applying MIGRATION_9: session task_id");
        let has_tid: bool = conn.query_row(
            "SELECT COUNT(*) > 0 FROM pragma_table_info('sessions') WHERE name='task_id'", [], |r| r.get(0))?;
        if !has_tid { conn.execute("ALTER TABLE sessions ADD COLUMN task_id INTEGER", [])?; }
        conn.execute_batch("BEGIN; INSERT INTO schema_version VALUES (9); COMMIT;")?;
    }

    if version < 10 {
        log::info!("[db/migrations] applying MIGRATION_10: session ended_at");
        let has_ended: bool = conn.query_row(
            "SELECT COUNT(*) > 0 FROM pragma_table_info('sessions') WHERE name='ended_at'", [], |r| r.get(0))?;
        if !has_ended { conn.execute("ALTER TABLE sessions ADD COLUMN ended_at INTEGER", [])?; }
        conn.execute_batch("BEGIN; INSERT INTO schema_version VALUES (10); COMMIT;")?;
    }

    if version < 11 {
        log::info!("[db/migrations] applying MIGRATION_11: seed default blocked sites");
        conn.execute_batch("BEGIN;
            INSERT OR IGNORE INTO settings (key, value) VALUES ('blocked_sites', 'youtube.com,twitter.com,x.com,reddit.com,facebook.com,instagram.com,tiktok.com,twitch.tv,netflix.com');
            INSERT OR IGNORE INTO settings (key, value) VALUES ('blocked_apps', '');
            INSERT OR IGNORE INTO settings (key, value) VALUES ('blocking_enabled', 'false');
