...
use commands::{
    ...
    task_list, task_create, task_set_active, task_complete, task_delete, task_update_estimate,
    task_list_completed, task_reopen,
    blocker_get_sites, blocker_set_sites, blocker_get_apps, blocker_set_apps, blocker_get_jar,
    jar_get_week,
    flow_get_state,
    flow_get_timeline,
    get_streak,
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            *tray_state.countdown_mode.lock().unwrap() = initial_settings.dial_countdown;
            ...
            // --- Flow tracker ---
            let flow_tracker = Arc::new(flow::FlowTracker::new(false));
            app.manage(Arc::clone(&flow_tracker));

            // --- App blocker ---
            let app_blocker = blocker::AppBlocker::new();
            app.manage(Arc::clone(&app_blocker));
            blocker::spawn_poll_thread(Arc::clone(&app_blocker), app.handle().clone());

            // --- Web blocker state ---
            let web_blocker = blocker::web::WebBlockerState::new();
            app.manage(Arc::clone(&web_blocker));

            // Emergency cleanup of stale proxy settings from previous crash
            blocker::web::emergency_cleanup();
