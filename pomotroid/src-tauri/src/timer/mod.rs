    ...
    while let Ok(event) = event_rx.recv() {
        match event {
            TimerEvent::Started { total_secs } => {
                ...
                // --- Flow + Blocker: activate on work start ---
                let is_work = sequence.lock().unwrap().current_round == crate::timer::sequence::RoundType::Work;
                if is_work {
                    if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let snap = ft.on_focus_start();
                        let _ = app.emit("flow:changed", &snap);
                        if let Some(ws) = app.try_state::<Arc<WsState>>() {
                            let _ = ws.broadcast_tx.send(websocket::WsEvent::FlowChanged { payload: snap });
                        }
                    }
                    if let Ok(conn) = db.lock() {
                        current_task_id = queries::task_get_active(&conn).ok().flatten();
                    }
                    let blocking_enabled = settings.lock().unwrap().blocking_enabled;
                    if blocking_enabled {
                        if let Some(blocker) = app.try_state::<Arc<crate::blocker::AppBlocker>>() {
                            let blocked_apps = settings.lock().unwrap().blocked_apps.clone();
                            let app_list: Vec<String> = blocked_apps.split('|').map(|s| s.to_string()).collect();
                            blocker.set_blocked_apps(&app_list);
                            blocker.start();
                        }
                        if let Some(wb) = app.try_state::<Arc<crate::blocker::web::WebBlockerState>>() {
                            if let Ok(conn) = db.lock() {
                                let sites = queries::get_blocked_sites(&conn).unwrap_or_default();
                                let wb2 = Arc::clone(&wb);
                                tauri::async_runtime::spawn(async move {
                                    crate::blocker::web::start(&wb2, sites).await;
                                });
                            }
                        }
                    }
            ...
            TimerEvent::Tick { elapsed_secs, total_secs } => {
                ...
                // Broadcast tick to WebSocket plugin every second.
                // The bridge silently stores it (no StateUpdated fired for ticks).
                if let Some(ws) = app.try_state::<Arc<WsState>>() {
                    let snap = build_snapshot(&sequence, &settings, &shared);
                    let _ = ws.broadcast_tx.send(websocket::WsEvent::State { payload: snap });
                }
                ...
                if elapsed_secs == 1 && current_session_id.is_none() {
                    ...
                    if let Ok(conn) = db.lock() {
                        match queries::insert_session_with_flow(&conn, &rt, total, current_task_id) {
                ...
                // --- Flow: on_tick every second during work, gradual decay during breaks ---
                if rt == "work" {
                    if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let snap = ft.on_tick();
                    ...
                    // During breaks: gradual decay every second, broadcast every 30s
                    if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let snap = ft.on_break_tick();
            ...
            TimerEvent::Complete { skipped: was_skipped } => {
                ...
                // --- Session recording with flow score ---
                if let Some(session_id) = current_session_id.take() {
                    let (flow_score, pause_count) = if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let snap = ft.snapshot();
                        (snap.score, snap.pause_count)
                    } else { (0, 0) };
                    if let Ok(conn) = db.lock() {
                        let _ = queries::complete_session_with_flow(&conn, session_id, !was_skipped, flow_score, pause_count);
                        // Broadcast jar immediately after session is written —
                        // before flow/task/blocker processing to eliminate lag.
                        if completed_round == "work" && !was_skipped {
                            if let Some(ws) = app.try_state::<Arc<WsState>>() {
                                if let Ok(entries) = queries::jar_today(&conn) {
                                    let payload = serde_json::to_value(&entries).unwrap_or_default();
                                    let _ = ws.broadcast_tx.send(websocket::WsEvent::JarChanged { payload });
                                }
                            }
                        }
                    }
                ...
                // --- Flow: handle work completion ---
                if completed_round == "work" {
                    if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let snap = if was_skipped { ft.on_focus_skipped() } else { ft.on_focus_complete() };
                        let _ = app.emit("flow:changed", &snap);
                        if let Some(ws) = app.try_state::<Arc<WsState>>() {
                            let _ = ws.broadcast_tx.send(websocket::WsEvent::FlowChanged { payload: snap });
                        }
                    }
                    // Increment task pomo on completion (not skip)
                    if !was_skipped {
                        if let Some(tid) = current_task_id {
                            if let Ok(conn) = db.lock() {
                                let _ = queries::task_increment_pomo(&conn, tid);
                                let work_secs = {
                                    let seq = sequence.lock().unwrap();
                                    let s = settings.lock().unwrap();
                                    seq.current_duration_secs(&s)
                                ...
                                let _ = conn.execute(
                                    "UPDATE tasks SET elapsed_work_secs = elapsed_work_secs + ?1 WHERE id = ?2",
                                    rusqlite::params![work_secs, tid],
                                );
                            }
                        }
                    }
                    // Stop blocker on work→break
                    if let Some(blocker) = app.try_state::<Arc<crate::blocker::AppBlocker>>() { blocker.stop(); }
                    if let Some(wb) = app.try_state::<Arc<crate::blocker::web::WebBlockerState>>() {
                        let wb2 = Arc::clone(&wb);
                        tauri::async_runtime::spawn(async move { crate::blocker::web::stop(&wb2).await; });
                    }
                }

                // --- Flow: handle break completion ---
                if completed_round == "short-break" || completed_round == "long-break" {
                    if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let elapsed = shared.lock().map(|s| s.elapsed_secs).unwrap_or(0);
                        let snap = if was_skipped {
                            if elapsed <= 10 {
                                if completed_round == "long-break" { ft.on_long_break_skipped() } else { ft.on_short_break_skipped() }
                            } else {
                                // Late skip — neutral, score preserved
                                ft.snapshot()
                            }
                        } else {
                            if completed_round == "long-break" { ft.on_long_break_taken() } else { ft.on_short_break_taken() }
                        };
                        // Always broadcast flow at break end so plugin stays in sync
                        let _ = app.emit("flow:changed", &snap);
                        if let Some(ws) = app.try_state::<Arc<WsState>>() {
                            let _ = ws.broadcast_tx.send(websocket::WsEvent::FlowChanged { payload: snap });
                        }
                ...
                if let Some(audio) = app.try_state::<Arc<AudioManager>>() {
                    if was_skipped {
                        // Skip feedback — play the sound for the phase we're skipping TO
                        let cue = match next_round {
                            RoundType::Work => AudioCue::SkipWork,
                            RoundType::ShortBreak => AudioCue::SkipShortBreak,
                            RoundType::LongBreak => AudioCue::SkipLongBreak,
                        };
                        audio.play_cue(cue);
                    } else {
                        audio.play_cue(AudioCue::PhaseComplete);
                    }
                }
            ...
            TimerEvent::Paused { elapsed_secs } => {
                ...
                // Flow: pause penalty (blocker stays ON during pause)
                if sequence.lock().unwrap().current_round == crate::timer::sequence::RoundType::Work {
                    if let Some(ft) = app.try_state::<Arc<crate::flow::FlowTracker>>() {
                        let snap = ft.on_pause();
                        let _ = app.emit("flow:changed", &snap);
                        if let Some(ws) = app.try_state::<Arc<WsState>>() {
                            let _ = ws.broadcast_tx.send(websocket::WsEvent::FlowChanged { payload: snap });
                        }
