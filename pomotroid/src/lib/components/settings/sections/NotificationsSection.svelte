<script lang="ts">
  ...
  const CUE_LIST: { id: CueKey; label: () => string }[] = [
    { id: 'phase_complete', label: () => 'Phase Complete' },
  ...
  let phaseComplete = $state<string | null>(null);
  let taskDone = $state<string | null>(null);

  function getFileName(id: CueKey): string | null {
    if (id === 'phase_complete') return phaseComplete;
    return taskDone;
  }

  function setFileName(id: CueKey, val: string | null) {
    if (id === 'phase_complete') phaseComplete = val;
    else taskDone = val;
  }
  ...
  async function refreshAudioInfo() {
    try {
      ...
      phaseComplete = info.phase_complete;
      taskDone = info.task_done;
    } catch (err) {
