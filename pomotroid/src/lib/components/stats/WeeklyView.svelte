<script lang="ts">
  ...
  import WeeklyJar from './WeeklyJar.svelte';

  let { week, streak, jarEntries = [] }: { week: DayStat[] | null; streak: StreakInfo | null; jarEntries?: WeekJarEntry[] } = $props();
  ...
  const dateStrings = $derived(days.map(d => d.date));
</script>
...
<div class="view">
  ...
  <!-- Weekly Jar — directly under chart -->
  <div class="jar-section-wrap">
    <WeeklyJar entries={jarEntries} dates={dateStrings} barW={BAR_W} barGap={BAR_GAP} />
  </div>
</div>
