...
export interface TaskItem {
  ...
  elapsed_work_secs: number;
  status: string;
  is_active: boolean;
  color: string;
}

export const taskList = () => invoke<TaskItem[]>('task_list');
export const taskCreate = (title: string, estimatedPomos: number) =>
  invoke<TaskItem>('task_create', { title, estimatedPomos });
export const taskSetActive = (id: number) => invoke<void>('task_set_active', { id });
export const taskComplete = (id: number) => invoke<void>('task_complete', { id });
export const taskDelete = (id: number) => invoke<void>('task_delete', { id });
export const taskUpdateEstimate = (id: number, estimatedPomos: number) =>
  invoke<void>('task_update_estimate', { id, estimatedPomos });
export const taskListCompleted = () => invoke<TaskItem[]>('task_list_completed');
export const taskReopen = (id: number) => invoke<void>('task_reopen', { id });

export async function onTasksChanged(cb: () => void): Promise<UnlistenFn> {
  return listen('tasks:changed', () => cb());
}

// --- Blocker commands ---

export const blockerGetSites = () => invoke<string[]>('blocker_get_sites');
export const blockerSetSites = (sites: string[]) => invoke<void>('blocker_set_sites', { sites });
export const blockerGetApps = () => invoke<string[]>('blocker_get_apps');
export const blockerSetApps = (apps: string[]) => invoke<void>('blocker_set_apps', { apps });
export interface WeekJarEntry {
  date: string;
  duration_secs: number;
  flow_score: number;
}

export const blockerGetJar = () => invoke<JarEntry[]>('blocker_get_jar');
export const jarGetWeek = () => invoke<WeekJarEntry[]>('jar_get_week');
...
export const getInstalledApps = () => invoke<InstalledApp[]>('get_installed_apps');
...
export const flowGetState = () => invoke<FlowState | null>('flow_get_state');
...
export const flowGetTimeline = () => invoke<FlowTimelineEntry[]>('flow_get_timeline');
...
export async function onFlowChanged(cb: (flow: FlowState) => void): Promise<UnlistenFn> {
  return listen<FlowState>('flow:changed', (e) => cb(e.payload));
}
