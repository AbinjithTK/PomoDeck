<script lang="ts">
  ...
  import ActiveTaskBar from '$lib/components/ActiveTaskBar.svelte';
  ...
  onMount(() => {
    ...
    (async () => {
      try {
        ...
        if (!localStorage.getItem('pomodeck_onboarding_done')) {
          showOnboarding = true;
        }
...
{#if showOnboarding}
  <Onboarding onDismiss={() => { showOnboarding = false; localStorage.setItem('pomodeck_onboarding_done', '1'); }} />
{/if}
