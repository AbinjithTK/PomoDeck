<script lang="ts">
  ...
  import TasksSection from '$lib/components/settings/sections/TasksSection.svelte';

  import * as m from '$paraglide/messages.js';

  type Section = 'timer' | 'tasks' | 'focus' | 'appearance' | 'notifications' | 'shortcuts' | 'system' | 'about';
  ...
  // Check URL param for initial section
  if (typeof window !== 'undefined') {
    const params = new URLSearchParams(window.location.search);
    const s = params.get('section');
    if (s && SECTIONS.some(sec => sec.id === s)) active = s as Section;
  }
  ...
  onMount(() => {
    ...
    (async () => {
      ...
      // Listen for section switch from titlebar focus button
      const { listen: listenEvent } = await import('@tauri-apps/api/event');
      cleanups.push(await listenEvent('switch-section', (e: any) => {
        const s = e.payload as string;
        if (SECTIONS.some(sec => sec.id === s)) active = s as Section;
      }));
    })();
