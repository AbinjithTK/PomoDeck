<script lang="ts">
  ...
  let weekJar = $state<import('$lib/ipc').WeekJarEntry[]>([]);

  // Check URL param for initial tab
  if (typeof window !== 'undefined') {
    const params = new URLSearchParams(window.location.search);
    const t = params.get('tab');
    if (t === 'week' || t === 'alltime') activeTab = t;
  }
  ...
  onMount(() => {
    ...
    (async () => {
      try {
        ...
        try { weekJar = await jarGetWeek(); } catch {}
      ...
      // Listen for tab switch from plugin
      const { listen: listenEvent } = await import('@tauri-apps/api/event');
      cleanups.push(await listenEvent('switch-tab', (e: any) => {
        const tab = e.payload as string;
        if (tab === 'today' || tab === 'week' || tab === 'alltime') switchTab(tab as Tab);
      }));
    })();
...
<div class="window">
  ...
  <div class="content">
    ...
    {:else if activeTab === 'week'}
      <WeeklyView week={detailed?.week ?? null} streak={detailed?.streak ?? null} jarEntries={weekJar} />
    {:else}
      <YearlyView {heatmap} />
    {/if}
  </div>
